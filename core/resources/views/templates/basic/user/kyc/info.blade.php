        ...
        <div class="card-body">
            @if ($user->kv == Status::KYC_PENDING)
                <div class="d-flex justify-content-center align-items-center mb-4 store-alert-message gap-2">
                    ...
                    <p class="text--warning">{{ __(@$kycContent->data_values->pending) }}</p>
                </div>
            @endif
            @if ($user->kyc_data)
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            ...
                            <span>
                                ...
                                @elseif($val->type == 'file')
                                    <a href="{{ route('user.download.attachment', encrypt(getFilePath('verify') . '/' . $val->value)) }}" class="text--base"><i class="fa fa-file"></i> @lang('Attachment') </a>
                                @else
                                    <p>{{ __($val->value) }}</p>
                                @endif
                            </span>
